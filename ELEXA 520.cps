/**
  ...
  Elexa post processor configuration.
...
description = "B15";
vendor = "Manuel Finessi";
vendorUrl = "";
legal = "";
certificationLevel = 2;
minimumRevision = 45702;

longDescription = "Post Processor per Elexa 520 - 3 Assi. Per C.B. Ferrari B15";
...
properties = {/*
  ...
  showNotes: {
    title      : "Scrivi note operazioni",
    description: "Scrive i commenti delle operazioni sul codice",
    type       : "boolean",
    value      : false,
    scope      : "post"
  },
  alzaZ: {
    title      : "A fine programma, alza la Z di",
    description: "Alza la Z a fine programma",
    type       : "integer",
    value      : 50,
    scope      : "post"
  }
...
*/
    ...
    writeBlock(gPlaneModal.format(30));

    writeBlock("G99 T" + toolFormat.format(tool.number) + " PA1");
    writeBlock(mFormat.format(13) + " S" + spindleSpeed);

    writeBlock(gPlaneModal.format(31));
...
function onCyclePoint(x, y, z) {
  ...
  if (isFirstCyclePoint()) {
    ...
    case "chip-breaking":
      ...
      break;
*/
    case "deep-drilling": // TODO: DA MODIFICARE
      writeBlock(
        gCycleModal.format(83),
        getCommonCycle(x, y, z, cycle.retract),
        "QZ-" + xyzFormat.format(cycle.incrementalDepth),
        /*conditional(K > 0, "K" + kFormat.format(K)), // optional*/
        feedOutput.format(F),
        "S" + spindleSpeed
      );
    ...
    case "right-tapping":
      ...
      writeBlock(
        ...
        getCommonCycle(x, y, z, cycle.retract),
        /*conditional(K > 0, "K" + kFormat.format(K)), // optional*/
        feedOutput.format(tool.threadPitch * 1000),
        "S" + spindleSpeed
      );
      break;
    case "reaming":
  ...
  if (x || y || z) {
    if (pendingRadiusCompensation >= 0) {
      ...
      case RADIUS_COMPENSATION_LEFT:
        writeBlock(gFormat.format(41), "D" + dFormat.format(d));
        writeBlock(gMotionModal.format(1), gFormat.format(41), x, y, z, f);
        break;
      case RADIUS_COMPENSATION_RIGHT:
        writeBlock(gFormat.format(42), "D" + dFormat.format(d));
...
function onLinear5D(_x, _y, _z, _a, _b, _c, feed) {
  if (!currentSection.isOptimizedForMachine()) {
    error(localize("5 ASSI NON IMPLEMENTATO NEL POST-PROCESSOR"));
    return;
  }
  if (pendingRadiusCompensation >= 0) {
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      ...
      break;
    case PLANE_ZX: // TODO: DA MODIFICARE 
      writeBlock(gPlaneModal.format(18));
      writeBlock(gMotionModal.format(clockwise ? 3 : 2), iOutput.format(cx), kOutput.format(cz), feedOutput.format(feed));
      break;
    case PLANE_YZ: // TODO: DA MODIFICARE 
    ...
    case PLANE_ZX:// TODO: DA MODIFICARE 
      ...
      writeBlock(gPlaneModal.format(18));
      writeBlock(gMotionModal.format(clockwise ? 3 : 2), xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cx), kOutput.format(cz), feedOutput.format(feed));
      break;
    case PLANE_YZ:// TODO: DA MODIFICARE 
      if (isHelical()) {
        linearize(tolerance);
...
function onSectionEnd() {
  if (currentSection.isMultiAxis()) {
    writeBlock(gMotionModal.format(41));
...
function onClose() {
  ...
  var az = getProperty("alzaZ");
  writeBlock(gMotionModal.format(0), zOutput.format(az));
